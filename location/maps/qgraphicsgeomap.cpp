...
{
  qDebug() << "QGraphicsGeoMap::addMapObject" << mapObject;
    if (!mapObject || !d_ptr->mapData)
        return;

    qDebug() << "QGraphicsGeoMap::addMapObject 1";
    if ((mapObject->type() == QGeoMapObject::CustomType) && !supportsCustomMapObjects())
        return;
    qDebug() << "QGraphicsGeoMap::addMapObject 2";
    d_ptr->mapData->addMapObject(mapObject);

    this->update();
    qDebug() << "QGraphicsGeoMap::addMapObject exit";
}
