...
void atDistanceAndAzimuth(const QGeoCoordinate &coord,
                                                 qreal distance, qreal azimuth,
                                                 double *lon, double *lat)
{
static const double qgeocoordinate_EARTH_MEAN_RADIUS = 6371.0072;

  double latRad = qgeocoordinate_degToRad(coord.latitude());
  double lonRad = qgeocoordinate_degToRad(coord.longitude());
                                   ...
                                   const ProjCoordinate &projCenter)
{
    ...
    for (double theta = startAngle; theta < stopAngle; theta += dth) {
        ...
        double x, y;
        atDistanceAndAzimuth(center, r, phiDeg,
                                                    &x, &y);
        x *= 3600.0;
        y *= 3600.0;
